...
async function sendReport(text, twyntId, userId, reporterId) {
	if (!client.isReady()) {
...
async function createReportEmbed(text, reportedtwynt, reportedUser, reporter) {
...
async function fetchtwyntInfo(twyntId) {
	try {
		const response = await fetch(`${API_BASE_URL}/api/twynt?id=${twyntId}`, {
			headers: { Authorization: ADMIN_KEY }
		});
		if (!response.ok) {
			throw new Error(`Failed to fetch twynt info: ${response.statusText} ${ADMIN_KEY}`);
		}
...
async function handleButtonPress(interaction, twyntId, userId) {
	try {
		let resultMessage = '';
