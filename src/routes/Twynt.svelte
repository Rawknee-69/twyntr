<script lang="ts">
	...
	import TwyntContents from './TwyntContents.svelte';
	...
	export let twyntClick: (id: string) => Promise<void>;
	...
	async function handleRepost() {
		...
		const response = await fetch('/api/liketwynt', {
			method: 'POST',
			body: JSON.stringify({ twyntId: id })
		});
		...
		if (response.status !== 200) {
			...
			toast(
				`Oh no! Something went wrong while liking the twynt. Error: ${response.status} | ${response.statusText}`
			);
		}
	}
	...
	async function opentwynt(twyntid: string) {
		twyntClick(twyntid);
	}
	...
	async function handlePost() {
		...
		const response = await fetch('api/twynt', {
			method: 'POST',
			body: formData
		});
		...
		if (response.status === 201) {
			openDialog = false;
			toast('Your twynt has been published!');
		} else {
...
<button on:click|stopPropagation={() => opentwynt(id)} class="mb-2 w-full text-left">
	<div
		class="flex w-full gap-3 rounded-xl bg-twynt-foreground p-3 transition-colors hover:bg-border"
	>
		...
		<div class="flex w-full max-w-[530px] flex-col gap-2 overflow-hidden">
			<!-- twynt that actually gets displayed. Main twynt -->
			<TwyntContents
				{truncateContent}
				postId={id}
				{bio}
			...
			{#if reposted && parentId}
				<button on:click|stopPropagation={() => opentwynt(parentId)}>
