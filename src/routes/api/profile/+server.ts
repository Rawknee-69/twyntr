...
import { followers, likes, twynts, notifications, users, history } from '@/server/schema';
import { eq, inArray, sql } from 'drizzle-orm';
import { minioClient } from '@/server/minio';
import { deletetwynt, uploadAvatar } from '../util';
...
export const POST: RequestHandler = async ({
	...
	try {
		...
		return json({ ...newtwynt, ...iqObject }, { status: 201 });
	} catch (error) {
...
export const DELETE: RequestHandler = async ({ request, cookies }) => {
	...
	try {
		...
		await db.transaction(async (tx) => {
			...
			console.time('Getting all twynts');
			const usertwynts = await tx
				.select({ id: twynts.id })
				.from(twynts)
				.where(eq(twynts.user_id, userId));
			console.timeEnd('Getting all twynts');
			console.time('Deleting all twynts');
			for (const twynt of usertwynts) {
				await deletetwynt(twynt.id);
			}
			console.timeEnd('Deleting all twynts');
			// Delete notifications
