			...
			try {
				const response = await fetch('/api/report', {
					...
					body: JSON.stringify({
						...
						twyntId // Use this for twynt reports
					})
				});
...
<Dialog.Root bind:open={opened}>
	<Dialog.Trigger>
		...
		>
			...
			<span>Report {profile ? 'profile' : 'twynt'}</span>
		</button>
	</Dialog.Trigger>
	<Dialog.Content class="sm:max-w-[425px]">
		<Dialog.Header>
			<Dialog.Title>Report {profile ? 'profile' : 'twynt'}.</Dialog.Title>
			<Dialog.Description>
				By submitting this report, you confirm that the reported content does not belong on Twyntr.
